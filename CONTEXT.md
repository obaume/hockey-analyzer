...
### Multi-game aggregate

Any stat computed over a hand-picked set of games rather than one: every count (attempts for/against, goals, zone starts, shots faced, time on ice) is summed across the selected games first, and percentages are then computed from those sums — never an average of per-game percentages, which would weight a two-shot game the same as a forty-shot one. The same sum-then-compute rule applies to **Position** rollups, which pool one team's skaters at a position (per team, never across both sides, since pooling both teams' defensemen would cancel each team's shots for against the other's). On-ice-attribution stats (individual and line stats) are summed only over each team's games that pass **Opponent shifts complete**, and the result names which selected games were included and excluded for each team; team-wide and goalie stats always cover every selected game. A line stat across games aggregates by unit slot (e.g. "Forward-Line 1"), even if who filled it changed from game to game.
